...
int writeFile(int disk, char* filename, void* block){  //block must be in heap
	...
	char* new_root_data = (char*)malloc(root_bytes+sizeof(struct dir_listing));
...
int deleteFile(int disk, char* filename){
	...
	memset(data1,0,BLOCK_SIZE);

	if(writeData(disk,inode->block[0],data1)==-1)
			return -1;
	
	
	inode->size = 0;
	inode->blocks = 0;
	memset(inode->block, 0 , 15*sizeof(inode->block[0]));
